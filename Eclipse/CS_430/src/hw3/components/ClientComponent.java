...
public class ClientComponent extends ThreadedComponent
{
  ...
  {
    // Copied from handleResult
    int id = msg.getCorrelationId();
    Integer key = pending.remove(id);
    if (key != null)
    {
      System.out.println("request for id "+ key +" timed out");
  ...
  private void doLookup(int key)
  {
    ...
    if (value == null)
    {
      ...
      pending.put(id, key);
      db.send(msg);
      /** Send to TimeoutComponent **/
      timer.send(new SetTimeoutMessage(this, id, Timeout));
    }
    else
    {
